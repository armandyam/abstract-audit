#!/usr/bin/env python3
"""
Per-paper sentence length metric — words per sentence in abstracts.
...
Method: count sentence-ending full-stops (period followed by whitespace, or
final period), then compute words_per_sentence = n_words / n_sentences.
This matches the narrator paper description: "tracking the frequency of
full-stops, selecting only for those used at the end of a sentence."

Output: data/per_paper/sentence_length/{venue}.csv
Columns: paper_id, venue, year, n_words, n_sentences, words_per_sentence

Usage:
  python src/metrics/sentence_length.py
  python src/metrics/sentence_length.py --venues neurips iclr
"""
...
from metrics._narrator_utils import (
    preprocess, word_count, count_sentences, iter_venue_papers,
)
    ...
    for fname in sorted(os.listdir(venue_dir)):
        ...
        for paper in papers:
            ...
            rows.append({
                ...
                "n_words":            nw,
                "n_sentences":        ns,
                "words_per_sentence": round(nw / ns, 4),
            })
        if rows:
...
def main():
    ...
    parser.add_argument("--out-dir",       default=os.path.join(repo, "data", "per_paper", "sentence_length"))
